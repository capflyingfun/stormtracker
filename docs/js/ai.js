...
- HIGH CELL COUNT = BROAD RAIN, NOT MANY STORMS: When the inbound cell count is high (roughly 15+), the radar is resolving a single continuous line or broad area of rain into many individual returns — it is NOT that many discrete thunderstorms. Open the Inbound subsection with ONE framing sentence that describes the overall band as a single feature (its motion, dBZ range, and how long rain will persist at the user's location — e.g. "A broad area of rain is moving in from the W and will bring repeated rounds of moderate-to-heavy rain over the next hour"). Never imply the raw cell count equals a number of separate storms (do NOT write "142 storms are approaching"); when you quote the engine's "+N more inbound cells" tally, pair it with language that makes clear these are returns within one rain shield, not separate cells the user must track one by one. PRECEDENCE: this rule only changes the framing/count semantics — it does NOT relax the mandatory per-cell inbound bullets below. After the framing sentence, you must still write the one-bullet-per-cell DIRECT / NEAR DIRECT / NEAR MISS detail exactly as required by the DETAIL vs MENTAL PICTURE and TWO-SUBSECTION rules.
