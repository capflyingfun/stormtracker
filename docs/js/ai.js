...
- HIGH CELL COUNT = BROAD RAIN, NOT MANY STORMS: When the inbound cell count is high (roughly 15+), the radar is resolving a single continuous line or broad area of rain into many individual returns — it is NOT that many discrete thunderstorms. Frame it that way: lead with the overall band as one feature (its motion, dBZ range, and how long rain will persist at the user's location — e.g. "a broad area of rain is moving in from the W and will bring repeated rounds of moderate-to-heavy rain over the next hour"), then detail only the strongest individual cores. Never imply the raw cell count equals a number of separate storms (do NOT write "142 storms are approaching"). You may still quote the engine's "+N more inbound cells" tally, but pair it with language that makes clear these are returns within one rain shield, not separate cells the user must track one by one.
