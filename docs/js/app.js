...
const TUTORIAL_SECTIONS=[
  ...
  {title:'🌍 Global Storm Finder',text:'Tap the <b>🌍 globe button</b> on the radar map (right side controls) to open the Global Storm Finder. This scans RainViewer radar across ~120 worldwide locations and uses AI to answer your question.<br><br><b>How it works:</b><br>1. Type a question like "Where are the strongest storms right now?" or "Any severe weather in Europe?"<br>2. The app downloads radar tiles worldwide and analyzes them for precipitation<br>3. AI interprets the radar data and presents matching storm hotspots<br>4. Each result has a <b>Go There →</b> button that flies you to that location<br><br><b>Quick buttons:</b> Tap any preset button for instant searches<br><br><b>Requirements:</b> OpenAI API key (same one used for AI Chat — set in Settings)<br><b>Note:</b> Results are cached for 5 minutes to save bandwidth'},
...
const CHANGELOG=[
  {ver:'v2.35',date:'2026-03-23',items:['🌍 Global Storm Finder — find storms anywhere in the world with natural language queries','Scans ~120 worldwide coordinates via RainViewer global radar tiles at zoom level 3','AI-powered analysis (GPT-4o-mini) interprets your query and matches it to live radar hotspots','Quick-search buttons: Strongest storms, US severe, Europe storms, Tropical activity, Asia rain','Each result shows region, peak dBZ, intensity category, and a "Go There →" button','Go There navigates to the storm location, sets it as your active location, and triggers a full radar scan','5-minute smart cache prevents redundant global scans','Globe 🌍 button added to radar map controls (right side)']},
  ...
  el.innerHTML=`
    ...
    <div class="map-container">
      ...
      <div class="map-controls map-controls-right">
        ...
        <div class="map-ctrl-btn" id="btn-global-finder" title="Global Storm Finder" style="font-size:0.85em" onclick="toggleGlobalFinder()">🌍</div>
...
// ==========================================
let _gsfOpen=false;
let _gsfScanning=false;
let _gsfCache={ts:0,hotspots:[]};

const GSF_SAMPLE_POINTS=[
  {lat:35,lon:-97,region:'Central US'},{lat:30,lon:-90,region:'Gulf Coast US'},{lat:40,lon:-75,region:'Northeast US'},
  {lat:33,lon:-84,region:'Southeast US'},{lat:45,lon:-93,region:'Upper Midwest US'},{lat:38,lon:-105,region:'Rocky Mountain US'},
  {lat:47,lon:-122,region:'Pacific NW US'},{lat:34,lon:-118,region:'Southern California'},{lat:25,lon:-80,region:'South Florida'},
  {lat:30,lon:-95,region:'Texas Gulf'},{lat:35,lon:-86,region:'Tennessee Valley'},{lat:42,lon:-83,region:'Great Lakes'},
  {lat:38,lon:-90,region:'Mississippi Valley'},{lat:32,lon:-110,region:'Arizona'},{lat:28,lon:-82,region:'Central Florida'},
  {lat:20,lon:-100,region:'Central Mexico'},{lat:19,lon:-99,region:'Mexico City'},{lat:25,lon:-100,region:'Northern Mexico'},
  {lat:15,lon:-90,region:'Guatemala'},{lat:10,lon:-84,region:'Costa Rica'},{lat:18,lon:-70,region:'Caribbean'},
  {lat:10,lon:-67,region:'Venezuela'},{lat:-3,lon:-60,region:'Amazon Basin'},{lat:-23,lon:-46,region:'São Paulo Brazil'},
  {lat:-15,lon:-47,region:'Central Brazil'},{lat:-34,lon:-58,region:'Buenos Aires'},{lat:-33,lon:-71,region:'Santiago Chile'},
  {lat:5,lon:-75,region:'Colombia'},{lat:-12,lon:-77,region:'Lima Peru'},
  {lat:51,lon:0,region:'London UK'},{lat:48,lon:2,region:'Paris France'},{lat:52,lon:13,region:'Berlin Germany'},
  {lat:41,lon:12,region:'Rome Italy'},{lat:40,lon:-4,region:'Madrid Spain'},{lat:55,lon:37,region:'Moscow Russia'},
  {lat:59,lon:18,region:'Stockholm Sweden'},{lat:52,lon:5,region:'Netherlands'},{lat:47,lon:8,region:'Switzerland'},
  {lat:50,lon:14,region:'Prague Czech'},{lat:48,lon:16,region:'Vienna Austria'},{lat:45,lon:25,region:'Romania'},
  {lat:60,lon:25,region:'Helsinki Finland'},{lat:38,lon:24,region:'Greece'},{lat:53,lon:-8,region:'Ireland'},
  {lat:42,lon:29,region:'Black Sea'},{lat:41,lon:29,region:'Istanbul Turkey'},
  {lat:30,lon:31,region:'Cairo Egypt'},{lat:6,lon:3,region:'Lagos Nigeria'},{lat:-1,lon:37,region:'Nairobi Kenya'},
  {lat:-26,lon:28,region:'Johannesburg SA'},{lat:34,lon:-7,region:'Morocco'},{lat:14,lon:-17,region:'Senegal'},
  {lat:9,lon:7,region:'Central Nigeria'},{lat:-4,lon:15,region:'Congo Basin'},{lat:-6,lon:35,region:'Tanzania'},
  {lat:0,lon:32,region:'Uganda'},{lat:-15,lon:35,region:'Malawi'},
  {lat:28,lon:77,region:'Delhi India'},{lat:19,lon:73,region:'Mumbai India'},{lat:13,lon:80,region:'Chennai India'},
  {lat:23,lon:90,region:'Bangladesh'},{lat:14,lon:101,region:'Bangkok Thailand'},{lat:1,lon:104,region:'Singapore'},
  {lat:-6,lon:107,region:'Jakarta Indonesia'},{lat:14,lon:121,region:'Manila Philippines'},
  {lat:35,lon:140,region:'Tokyo Japan'},{lat:37,lon:127,region:'Seoul Korea'},{lat:31,lon:121,region:'Shanghai China'},
  {lat:23,lon:113,region:'Guangzhou China'},{lat:40,lon:116,region:'Beijing China'},{lat:22,lon:114,region:'Hong Kong'},
  {lat:16,lon:106,region:'Vietnam'},{lat:22,lon:96,region:'Myanmar'},{lat:28,lon:84,region:'Nepal'},
  {lat:25,lon:67,region:'Karachi Pakistan'},{lat:34,lon:69,region:'Kabul Afghanistan'},
  {lat:33,lon:44,region:'Baghdad Iraq'},{lat:36,lon:52,region:'Iran'},{lat:24,lon:47,region:'Saudi Arabia'},
  {lat:32,lon:35,region:'Israel/Palestine'},{lat:40,lon:44,region:'Caucasus'},
  {lat:-34,lon:151,region:'Sydney Australia'},{lat:-28,lon:153,region:'Queensland Australia'},
  {lat:-38,lon:145,region:'Melbourne Australia'},{lat:-12,lon:131,region:'Northern Australia'},
  {lat:-37,lon:175,region:'New Zealand'},{lat:-16,lon:178,region:'Fiji'},
  {lat:21,lon:-158,region:'Hawaii'},{lat:65,lon:-18,region:'Iceland'},{lat:64,lon:26,region:'Finland North'},
  {lat:10,lon:-10,region:'West Africa'},{lat:-8,lon:-35,region:'NE Brazil'},
  {lat:26,lon:127,region:'Okinawa Japan'},{lat:15,lon:-61,region:'Lesser Antilles'},
  {lat:25,lon:-77,region:'Bahamas'},{lat:23,lon:-82,region:'Cuba'},{lat:45,lon:-64,region:'Maritime Canada'},
  {lat:-20,lon:47,region:'Madagascar'},{lat:-22,lon:167,region:'New Caledonia'},
  {lat:55,lon:-3,region:'Scotland UK'},{lat:46,lon:7,region:'Alps'},{lat:43,lon:42,region:'Caucasus Mtns'},
  {lat:5,lon:100,region:'Malaysia'},{lat:-2,lon:120,region:'Sulawesi Indonesia'},
  {lat:35,lon:136,region:'Central Japan'},{lat:30,lon:120,region:'East China'},
  {lat:45,lon:135,region:'Russian Far East'},{lat:50,lon:80,region:'Central Asia'},
  {lat:-5,lon:30,region:'Lake Victoria'},{lat:12,lon:43,region:'Horn of Africa'},
  {lat:10,lon:80,region:'Sri Lanka'},{lat:15,lon:46,region:'Yemen'},
  {lat:-10,lon:-55,region:'Central Brazil'},{lat:-30,lon:-51,region:'Southern Brazil'},
  {lat:44,lon:-79,region:'Southern Ontario'},{lat:49,lon:-97,region:'Manitoba Canada'},
  {lat:51,lon:-114,region:'Alberta Canada'},{lat:49,lon:-123,region:'Vancouver Canada'},
  {lat:18,lon:-66,region:'Puerto Rico'},{lat:32,lon:-64,region:'Bermuda'}
];

async function globalRadarScan(){
  if(_gsfCache.ts&&Date.now()-_gsfCache.ts<5*60000)return _gsfCache.hotspots;
  _gsfScanning=true;
  updateGsfUI();
  try{
    const rvRes=await fetch('https://api.rainviewer.com/public/weather-maps.json',{signal:AbortSignal.timeout(8000)});
    if(!rvRes.ok)throw new Error('RainViewer unavailable');
    const rvData=await rvRes.json();
    const frame=rvData.radar?.past?.slice(-1)[0];
    if(!frame)throw new Error('No radar frames');
    const zoom=3;
    const tileMap={};
    GSF_SAMPLE_POINTS.forEach(pt=>{
      const tx=Math.floor((pt.lon+180)/360*Math.pow(2,zoom));
      const n=Math.pow(2,zoom);
      const latR=pt.lat*Math.PI/180;
      const ty=Math.floor((1-Math.log(Math.tan(latR)+1/Math.cos(latR))/Math.PI)/2*n);
      const key=`${tx}_${ty}`;
      if(!tileMap[key])tileMap[key]={tx,ty,points:[]};
      tileMap[key].points.push(pt);
    });
    const hotspots=[];
    ...
    for(let i=0;i<tiles.length;i+=batchSize){
      ...
      const results=await Promise.allSettled(batch.map(t=>scanGsfTile(t,frame,zoom)));
      results.forEach(r=>{if(r.status==='fulfilled'&&r.value)hotspots.push(...r.value)});
      const pct=Math.min(100,Math.round((i+batchSize)/tiles.length*100));
      const prog=document.getElementById('gsf-progress');
      if(prog)prog.textContent=`Scanning global radar... ${pct}%`;
    }
    hotspots.sort((a,b)=>b.dbz-a.dbz);
    _gsfCache={ts:Date.now(),hotspots};
    _gsfScanning=false;
    updateGsfUI();
    return hotspots;
  }catch(e){
...
async function scanGsfTile(tile,frame,zoom){
  const url=`https://tilecache.rainviewer.com/v2/radar/${frame.time}/256/${zoom}/${tile.tx}/${tile.ty}/2/1_1.png`;
  try{
    ...
    for(let x=0;x<w;x+=step){
      for(let y=0;y<h;y+=step){
        ...
        const ptLat=ptLatRad*180/Math.PI;
        results.push({lat:ptLat,lon:ptLon,dbz});
      }
    }
    if(results.length===0)return[];
    const clusters=clusterGsfPoints(results,tile.points);
    return clusters;
  }catch(e){return[]}
}

function clusterGsfPoints(points,samplePts){
  const clusters=[];
  const grid={};
  const cellSize=1.5;
  points.forEach(p=>{
    ...
    const key=`${gx}_${gy}`;
    if(!grid[key])grid[key]={pts:[],maxDbz:0,sumLat:0,sumLon:0,sumDbz:0};
    grid[key].pts.push(p);
    if(p.dbz>grid[key].maxDbz)grid[key].maxDbz=p.dbz;
    grid[key].sumLat+=p.lat;grid[key].sumLon+=p.lon;grid[key].sumDbz+=p.dbz;
  });
  Object.values(grid).forEach(g=>{
    if(g.pts.length<2)return;
    const cLat=g.sumLat/g.pts.length;
    const cLon=g.sumLon/g.pts.length;
    let region='Unknown';
    let minD=Infinity;
    samplePts.forEach(sp=>{
      const d=Math.abs(sp.lat-cLat)+Math.abs(sp.lon-cLon);
      if(d<minD){minD=d;region=sp.region}
    });
    if(minD>15){
      GSF_SAMPLE_POINTS.forEach(sp=>{
        const d=Math.abs(sp.lat-cLat)+Math.abs(sp.lon-cLon);
        if(d<minD){minD=d;region=sp.region}
      });
    }
    clusters.push({lat:cLat,lon:cLon,dbz:g.maxDbz,avgDbz:Math.round(g.sumDbz/g.pts.length),count:g.pts.length,region});
  });
  return clusters;
}

function toggleGlobalFinder(){
  _gsfOpen=!_gsfOpen;
  const el=document.getElementById('gsf-overlay');
  if(el)el.style.display=_gsfOpen?'flex':'none';
  if(_gsfOpen){
    setTimeout(()=>{const inp=document.getElementById('gsf-input');if(inp)inp.focus()},200);
  }
}

function updateGsfUI(){
  const prog=document.getElementById('gsf-progress');
  const btn=document.getElementById('gsf-search-btn');
  if(_gsfScanning){
    if(prog)prog.style.display='block';
    if(btn){btn.disabled=true;btn.textContent='Scanning...';}
  }else{
    if(prog)prog.style.display='none';
    if(btn){btn.disabled=false;btn.textContent='🔍 Find Storms';}
  }
}

async function gsfSearch(){
  const inp=document.getElementById('gsf-input');
  if(!inp)return;
  ...
  const resultsEl=document.getElementById('gsf-results');
  if(resultsEl)resultsEl.innerHTML='<div style="text-align:center;padding:20px;color:var(--text-muted)"><div style="font-size:1.5em;margin-bottom:8px">🛰️</div>Scanning global radar...</div>';
  const hotspots=await globalRadarScan();
  if(hotspots.length===0){
    if(resultsEl)resultsEl.innerHTML='<div style="text-align:center;padding:20px;color:var(--text-muted)">No significant precipitation detected worldwide right now.</div>';
    return;
  }
  if(resultsEl)resultsEl.innerHTML='<div style="text-align:center;padding:20px;color:var(--text-muted)"><div style="font-size:1.5em;margin-bottom:8px">🤖</div>AI analyzing results...</div>';
  const top30=hotspots.slice(0,30);
  const hsStr=top30.map(h=>`${h.region}: ${h.dbz}dBZ peak, ${h.avgDbz}dBZ avg, ${h.count} cells at (${h.lat.toFixed(2)},${h.lon.toFixed(2)})`).join('\n');
  try{
    const sysMsg=`You are a meteorologist storm finder. The user asked: "${query}". Below is live global radar data showing active precipitation hotspots (sorted by intensity). Analyze these hotspots and answer the user's query. For each relevant result, output it in this exact format on its own line:
[STORM_RESULT]{"lat":NUMBER,"lon":NUMBER,"dbz":NUMBER,"region":"NAME","summary":"SHORT_DESCRIPTION"}[/STORM_RESULT]
After the results, provide a brief conversational analysis. If the query asks about a specific region, filter to that area. If it asks for the strongest storms, show the top ones. Always include at least the most relevant results.

GLOBAL RADAR HOTSPOTS (${new Date().toUTCString()}):
${hsStr}`;
    const res=await fetch('https://api.openai.com/v1/chat/completions',{
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':'Bearer '+key},
      body:JSON.stringify({model:'gpt-4o-mini',messages:[{role:'system',content:sysMsg},{role:'user',content:query}],max_tokens:1500,temperature:0.3})
    });
    if(!res.ok){
...
function gsfGoThere(lat,lon,name){
  toggleGlobalFinder();
  setLoc(lat,lon,name||`${lat.toFixed(2)}, ${lon.toFixed(2)}`);
  switchPage('radar');
  if(S.map){
    setTimeout(()=>{S.map.flyTo([lat,lon],7,{duration:1.5})},300);
  }
