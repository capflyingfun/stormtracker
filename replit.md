...
- **Core Features**:
    ...
    - **Alert System**: Personalized, real-time risk alerts based on storm intensity, distance, and lightning. Includes visual storm highlighting, NWS alerts integration with chronological sorting, and an automated threat detection system. Features impact threshold filter (0-85% adjustable in Storm Filtering settings, localStorage-persisted), color-coded alert borders (red 75%+, orange 50%+, amber 25%+, purple for extreme dBZ), and live countdown timers (⏱️ ETA) for approaching storms.
