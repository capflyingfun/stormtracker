...
- **Core Features**:
    ...
    - **Global Storm Finder**: Find storms anywhere in the world via natural language queries. Globe 🌍 button on radar map opens finder overlay. Scans ~120 worldwide coordinates via RainViewer global radar tiles (zoom level 3) with canvas-based PNG decoding and dBZ color mapping. AI-powered analysis (GPT-4o-mini) interprets user queries against live radar hotspots. Quick-search preset buttons (Strongest storms, US severe, Europe storms, Tropical, Asia rain). Each result shows region, peak/avg dBZ, intensity category, and "Go There →" button. Go There sets location, flies map, triggers full radar scan. 5-minute smart cache. Requires user's OpenAI API key (same as AI Chat). Functions: `globalRadarScan()`, `scanGsfTile()`, `clusterGsfPoints()`, `gsfSearch()`, `renderGsfResults()`, `gsfGoThere()`.
