...
import type { InsertMessageInbox } from '@shared/schema';
...
// Built-in database message storage function
export async function storeMessageInDatabase(params: StormAlertEmailParams): Promise<boolean> {
  try {
    ...
    const textContent = `
⚡ STORM ALERT: ${intensityCategory} storm detected

Location: ${params.locationName}
Intensity: ${params.stormIntensity} dBZ (${intensityCategory})
Distance: ${params.stormDistance.toFixed(1)} miles ${params.stormDirection}
Impact Chance: ${params.impactChance}
Severity: ${params.severity}
${params.eta ? `ETA: ${params.eta}` : ''}

${params.stormIntensity >= 55 ? 'TAKE SHELTER IMMEDIATELY - Severe storm conditions' : 
  params.stormIntensity >= 45 ? 'Stay indoors - Heavy precipitation expected' : 
  'Monitor conditions - Light to moderate precipitation'}

This alert was generated by StormTracker for ${params.locationName}.
Stay safe!
    `.trim();

    // Store email message in database
    const emailMessage: InsertMessageInbox = {
      ...
      recipientName: params.to.split('@')[0], // Use email username as name fallback
      stormCount: 1,
      maxIntensity: params.stormIntensity,
      nearestDistance: params.stormDistance,
      alertLocation: params.locationName,
      deliveryMethod: 'database',
      isDelivered: true
    };

    await storage.createMessage(emailMessage);
    console.log(`Storm alert stored in database for ${params.to} - ${intensityCategory} storm at ${params.stormDistance.toFixed(1)} miles`);
    return true;
...
export async function sendStormAlert(params: StormAlertEmailParams): Promise<boolean> {
  // First, always store in database for built-in inbox system
  const databaseStored = await storeMessageInDatabase(params);
  
  // If no external email service is configured, just use database storage
  if (!hasEmailService) {
    console.log(`Storm alert stored in built-in inbox for ${params.to}`);
    return databaseStored;
  }
