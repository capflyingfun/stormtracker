  ...
  private calculateActivationStatus(effective: string, alertType: string): string {
    try {
      ...
      if (now >= effectiveDate) {
        ...
        const timeUntil = effectiveDate.getTime() - now.getTime();
        const hoursUntil = Math.floor(timeUntil / (1000 * 60 * 60));
        const minutesUntil = Math.floor((timeUntil % (1000 * 60 * 60)) / (1000 * 60));
        
        if (alertType.includes('Heat Advisory')) {
          return hoursUntil > 0 ? `Begins in ${hoursUntil} hours` : `Begins in ${minutesUntil} minutes`;
        }
        
        return hoursUntil > 0 ? `Activates in ${hoursUntil} hours` : `Activates in ${minutesUntil} minutes`;
      }
    } catch (error) {
      console.error('Activation status calculation error:', error);
      return 'Unknown';
    }
  }
