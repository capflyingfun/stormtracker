...
class ThreatDetectionService {
  ...
  async performThreatDetection(params: {
    lat: number;
    lon: number;
    address: string;
    storms?: any[];
    lightningCount?: number;
  }): Promise<{ threatCount: number; threats: DetectedThreat[] }> {
    const userLocation: UserLocation = {
      lat: params.lat,
      lon: params.lon,
      address: params.address
    };
    const storms = params.storms || [];
    const lightningCount = params.lightningCount || 0;
    ...
    const threats = await this.detectThreats(userLocation, storms, weatherData, lightningCount);
    return { threatCount: threats.length, threats };
  }
  ...
  private calculateTimeToExpiration(expires: string): string {
    try {
      ...
      console.error('Time to expiration calculation error:', error);
      return 'Unknown';
    }
  }
  ...
  private calculateActivationStatus(effective: string, alertType?: string): string {
    try {
      ...
      if (alertType && alertType.toLowerCase().includes('heat')) {
        ...
        const alertEffectiveDate = new Date(effective);
        console.log(`🗓️ Parsing Heat Advisory for effective date: ${alertEffectiveDate.toDateString()}`);

        // For Heat Advisories, start at 10 AM on the effective date
        const alertLocal = new Date(alertEffectiveDate);
  ...
  private async detectLightningThreats(userLocation: UserLocation, lightningCount: number): Promise<DetectedThreat[]> {
    ...
    if (lightningCount >= 5) {
      ...
      // Check extreme threshold first, then high (order matters)
      if (lightningCount >= 50) {
        threatLevel = 'extreme';
        riskToPublic = 'extreme';
        priority = 1;
